## Title
> TrailBuddy

## Team Members
> Bryan Palma-Matos

 

> Sarah Hyatt

 

## Description 
TrailBuddy is a platform that takes away the hassle of going through many searches for an outdoor activity that may not even be what the user is looking for.
TrailBuddy's system supports users being able to curate their searches based on their likings and signing up for them on the same platform. TrailBuddy also supports providers to upload, update, and remove their listings as needed. TrailBuddy makes the process of exploring outdoor activities based on tailored needs, signing up, and leaving feedback a whole lot easier. 

## App Functions
1. Customer (the user with the customer role):
    1. Create/modify customer profile - Register an account via a login screen as well and display/edit their skill level and preferences.
    2. View available services - View a variety of activities, guides, classes, or events with a wide array of skill levels for the user to choose from.
    3. Subscribe to available services - The user will be able to select one of the available services and register themselves for said outdoor activity.
    4. Write reviews for subscribed services - Users will be able to leave a written review about the activity they had selected as well as be able to view other reviews left on that page.
2. Provider (Individual Guides or Organizations):
    1. Create/modify/remove provider profile - Register as a provider and display overall experience and specialties.
    2. Create/update/remove services - Publish offerings for classes, guided hikes, or outdoor events for customers.
    3. View customer statistics - See how many people have registered for their service and see the profile of those who have registered.
    4. Reply to reviews - Respond to customer feedback in a professional manner.
